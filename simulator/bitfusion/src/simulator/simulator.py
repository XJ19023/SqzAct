...
from bitfusion.src.simulator.globalVar import get_hw_arch
class Simulator(object):
    ...
    def get_energy_cost(self):
        ...
        if len(core_data) == 0:
            ...
            core_leak_energy = core_leak_power / float(core_data['Frequency'].iloc[0])
        # '''
        # core_data = core_synth_data.loc[core_synth_data['hw'] == get_hw_arch()]
        # # with open('123.log', 'a') as f:
        # #     f.writelines(f'{get_hw_arch()}={core_data}\n M={M} N={N}\n')
        # core_area = float(core_data['Area (um^2)'].iloc[0]) * 1.e-6 * (N * M) / 1.
        # core_dyn_power = float(core_data['Dynamic Power (nW)'].iloc[0]) * (N * M) / 1.
        # core_dyn_energy = core_dyn_power / float(core_data['Frequency'].iloc[0])
        # core_leak_power = float(core_data['Leakage Power (nW)'].iloc[0]) * (N * M) / 1.
    ...
    def get_FC_cycles(self, Ni, No,
                      ...
                      int8_ratio=1):
        # with open('123.log', 'a') as f:
        #     f.writelines(f'Ni={Ni}, No={No}, iprec={iprec}, wprec={wprec}, batch_size={batch_size}\n')
        """
        ...
        """
        total_cycles = self.get_conv_cycles(1, 1, 1, Ni, No, iprec, wprec, batch_size, im2col=True, weight_stationary = weight_stationary, int8_ratio=int8_ratio)
    ...
    def get_conv_cycles(self, K, O, S, IC, OC, iprec, wprec, batch_size=1, im2col=True, weight_stationary = False, int8_ratio=1):
        """
        ...
        conv_params = self.accelerator, K, O, S, IC, OC, B, iprec, wprec, im2col, weight_stationary, self.get_energy_cost(), int8_ratio
        ...
        # print(best_order)
        # with open('123.log', 'a') as f:
        #     f.writelines(f'>> {best_tiling}\n')


        stats = get_stats_fast(conv_params, best_tiling, best_order, verbose=False, final=True)
    ...
    def get_cycles(self, op, im2col=True, weight_stationary = False):
        if isinstance(op, Convolution):
            ...
            '''
            with open('123.log', 'a') as f:
                f.writelines(f'{op.name} int8_ratio={op.int8_ratio} ')
            '''
            ...
            iprec = op.data.dtype.bits
            wprec = op.weights.dtype.bits
            return self.get_FC_cycles(IC, OC, iprec, wprec, batch_size=B, weight_stationary = weight_stationary, int8_ratio=op.int8_ratio)
