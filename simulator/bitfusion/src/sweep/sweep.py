...
class SimulatorSweep(object):
    ...
    def sweep(self, sim_obj, list_n=None, list_m=None, list_pmax=None, list_pmin=None, list_bw=None, list_bench=None, list_wbuf=None, list_ibuf=None, list_obuf=None, list_batch=None, bench_type = None, weight_stationary = True):
        ...
        for b in list_bench:
            ...
            elif bench_type == 'mant':
                nn = benchmarks.get_bench_nn_mant(b, batch_size)
            elif bench_type == 'olive':
                nn = benchmarks.get_bench_nn_olive(b, batch_size)
            elif bench_type == 'spark':
                nn = benchmarks.get_bench_nn_spark(b, batch_size)
            elif bench_type == 'squeeze':
                nn = benchmarks.get_bench_nn_squeeze(b, batch_size)
                print(nn)
            if len(results) == 0:
