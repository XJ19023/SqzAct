...
def get_stats_fast(conv_params, tiling, order_type, verbose=False, final=False):
    """
    ...
    """
    acc_obj, K, O, S, IC, OC, B, iprec, wprec, im2col, weight_stationary, energy_cost, int8_ratio = conv_params
    ...
    if get_run_mode() in ['qwt', 'reorder']: # Our method
        wrt_wgt_int4 = \
                ceil_a_by_b(K * K * oc, acc_obj.N * acc_obj.get_perf_factor(wprec)) * acc_obj.N * acc_obj.get_perf_factor(wprec) * \
                ic * \
                wprec
        wrt_act_int8 = ow * oh * \
                ceil_a_by_b(K * K * b, acc_obj.M * acc_obj.get_perf_factor(iprec)) * acc_obj.M * acc_obj.get_perf_factor(iprec) * \
                ic * iprec
        wrt_act_int4 = wrt_act_int8 / 2
        writes['wgt'] = wrt_wgt_int4
        writes['act'] = wrt_act_int8 * int8_ratio + wrt_act_int4 * (1 - int8_ratio)
    else:   # Default in ANT framework
        writes['wgt'] = \
                ceil_a_by_b(K * K * oc, acc_obj.N * acc_obj.get_perf_factor(wprec)) * acc_obj.N * acc_obj.get_perf_factor(wprec) * \
                ic * \
                wprec
    ...
    if weight_stationary:
        ...
        if get_run_mode() in ['qwt', 'reorder']: # Our method
        # if get_run_mode() in ['test']: # Our method
            rds_wgt_int4 = num_tiles * (oc * oh * ow * b) * (kw * kh * ic) * wprec
            rds_act_int8 = num_tiles * (oc * oh * ow * b) * (kw * kh * ic) * iprec
            rds_act_int4 = rds_act_int8 / 2
            stats.reads['wgt'] += rds_wgt_int4
            stats.reads['act'] += (rds_act_int8 * int8_ratio + rds_act_int4 * (1 - int8_ratio))
        else: # Default in ANT framework
    ...
    if weight_stationary:
        ...
        compute_cycles = num_tiles * acc_obj.get_compute_cycles_output_stationary(ic, oc, ow, oh, b, kw, kh, iprec, wprec, im2col, final, int8_ratio)
...
def optimize_for_order(conv_params):
    ...
    return_dict = {}
    acc_obj, K, O, S, IC, OC, B, iprec, wprec, im2col, weight_stationary, energy_cost, _ = conv_params
...
def get_loop_instructions(conv_params, tiling, order_type):
    ...
    if im2col:
        wgt_read_size = \
                ceil_a_by_b(K * K * oc, acc_obj.N) * acc_obj.N * ic * \
                wprec
        max_wgt_size = \
